...
//	    // 토심이: "마지막 수정자도 추적 가능합니다!"
		...
		// -> 사용자 인증이 유효한 시간 자체를 명시할 수 있음
		// -> 유효 시간이 정해지면 브라우저가 종료되어도 인증이 유지된다는 특징이 있음
		// -> 클라이언트의 상태정보를 로컬에 저장했다가 참조
		
		// 세션: 쿠키랑 비슷한데 저장 위치가 서비스 제공하는 서버측임
		//		세션 ID라는 것을 부여한 후 서버에 접속해서 종료할 때까지
		// 		인증 상태를 유지하는 것이 일반적
		...
		// @ControllerAdvice
		//  -> 모든 컨트롤러를 중앙 집중 식으로 관리하는 어노테이션 (특정한 범위로 제한도 가능)
		//     -> 제한 범위 : 해당 컨트롤러 파일, 해당 패키지, 상속 관계
		// 	-> 컨트롤러에서 공통적으로 발생하는 예외, 로직을 중앙에서 처리할 수 있게 해줌
